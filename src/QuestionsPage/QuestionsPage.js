...
function QuestionsPage() {
    ...
    return (
        <div className='questions_root'>
            <Header/>
            <p className="questions_title">Здесь представлены частые вопросы </p>
            ...
            <div className="questions_main_div">
                ...
                <div id={`answer-${1}`} className={`questions_answer ${openQuestions[1] ? 'opened' : ''}`}>
                    <ul>
                        <li>Инженерный расчёт конструкции дома.</li>
                        <li>Фундамент монолитно-ленточный, Н-1200 мм, В-300 мм.</li>
                        <li>Подготовка вводов/выводов инженерных коммуникаций: канализация, водопровод, электричество.
                        </li>
                        <li>Строительно-монтажный комплект дома: набор панелей по согласованному проекту,
                            антисептированный брус,
                            антисептическая обработка древесины методом консервации.
                        </li>
                        ...
                        <li>Кровля (кровельная СИП-панель, паропропускная пленка, контрбрус, обрешетка,
            ...
            <div className="questions_main_div">
                ...
                <div id={`answer-${3}`} className={`questions_answer ${openQuestions[3] ? 'opened' : ''}`}>
                    <p>С отоплением дома, построенного по СИП-технологии, справится практически любая система, будь то
            ...
            <div className="questions_main_div">
                ...
                <div id={`answer-${6}`} className={`questions_answer ${openQuestions[6] ? 'opened' : ''}`}>
                    <p>Из чего состоит СИП-панель? Прежде всего, это наружный и внутренний слой OSB-плиты толщиной 12
                        ...
                        конструкций. Толщина пенополистирола, используемого в изготовлении СИП-панелей, от 150 до 200
                        мм. Склеивается СИП-панель под давлением. Задача OSB-плит в СИП-панели — обеспечение прочности.
                        Благодаря такой технологии СИП-панель выдерживает продольную нагрузку порядка 8 тонн на всю
                        ширину 1,25 м, поперечная нагрузка допускается порядка одной тонны на равномерно распределённый
                        1 м2. За границей допускается использование СИП-панелей для строительства 5-этажных домов, в
                        нашей же стране действует ограничение до 2-этажного дома и мансарды.</p>
                </div>
            </div>
            <div className="questions_main_div">
                ...
                <div id={`answer-${7}`} className={`questions_answer ${openQuestions[7] ? 'opened' : ''}`}>
                    <p>Пенополистирол — это нейтральный, инертный слой в СИП-панели, который не вреден для здоровья
                        ...
                        легко — дома из СИП-панелейгипоаллергенны. В развитых странах использование пенополистирола в
            ...
            <div className="questions_main_div">
                ...
                <div id={`answer-${8}`} className={`questions_answer ${openQuestions[8] ? 'opened' : ''}`}>
                    <p>Дом из СИП-панелей обладает высокой степенью сопротивлению горения, входящий в состав
                        пенополистирола «антипирен» наделяет панель самозатухающим эффектом. Температура воспламенения
                        ...
                        сажа, которая не является токсичной, пенополистирол дополнительно защищен плитами OSB-3,
                        которые, в свою очередь, не подвержены к горению благодаря обработке огнебиозащитным составом.
                        После сборки Вашего дома, при внутренней отделке, как правило, используется гипсокартон, для
                        монтажа которого не требуется обрешетка с воздушными прослойками, повышающими распространение
                        огня, гипсокартон достаточно крепить прямо к СИП-панели, отсутствие воздуха затруднит
                        распространение огня, что, в свою очередь, также повышает и без того высокие противопожарные
                        характеристики дома. В целом это даёт сопротивляемость открытому огню порядка 45 минут.</p>
                </div>
            </div>
            <div className="questions_main_div">
                ...
                <div id={`answer-${9}`} className={`questions_answer ${openQuestions[9] ? 'opened' : ''}`}>
                    <p>Пенополистирол — продукт небиологический, для мышей он несъедобен. Как показывает практика, мыши
                        ...
                        любой другой, есть возможность дополнительной обработки дома от грызунов специальным составом,
                        не вредным для человека, нижних панелей, либо использовать сетку с мелкой ячейкой, также полезны
            ...
            <div className="questions_main_div">
                ...
                <div id={`answer-${13}`} className={`questions_answer ${openQuestions[13] ? 'opened' : ''}`}>
                    <p>Конечно! Если у вас нет возможности присутствовать на объекте, по вашему запросу мы предоставляем
                        фото- и видеоматериалы. Можем осуществлять онлайн-трансляцию, заранее установив камеры и
            ...
            <div className="questions_main_div">
                ...
                <div id={`answer-${14}`} className={`questions_answer ${openQuestions[14] ? 'opened' : ''}`}>
                    <p>Мы работаем круглогодично. У нас есть отработанные технологии и все необходимое оборудование для
                        строительства в летний и зимний период, в самые злющие морозы будем отдыхать 😊</p>
                </div>
            </div>
